...
parser.add_argument('--nb_protos', default=2, type=int, \
...
for iteration_total in range(args.nb_runs):
    ...
    for orde in range(args.num_classes):
        A.append(len(np.where(Y_train_total==order[orde])[0]))
    for orde in range(args.num_classes):
    # aaa = A[orde][0].shape
        prototypes = np.zeros((args.num_classes,A[orde],X_train_total.shape[1],X_train_total.shape[2],X_train_total.shape[3]))
    # A.append(np.where(Y_train_total==order[orde]))
